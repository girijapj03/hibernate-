...
public class MobileDAOImple implements MobileDAO{
	...
	public void updatePriceByColor() {
		try (Session session = factory.openSession()) {
			Query query = session
					.createQuery("update MobileEntity as mobile set mobile.price='23500.0'where mobile.color='Purple'");
	...
	public void deleteRowById() {
		try (Session session = factory.openSession()) {
			Query query = session.createQuery("delete MobileEntity  mobile where mobile.id=2");
			session.beginTransaction();
			query.executeUpdate();
			session.getTransaction().commit();
	...
	public double getMobilePriceByBrand(String brand) {
		...
		try (Session session = factory.openSession()) {
			Query query = session.createQuery("select mobile.price from MobileEntity mobile where mobile.brand='+brand+'");
